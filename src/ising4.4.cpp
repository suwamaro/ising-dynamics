...
double calc_eff_local_field(bool fast_code, std::size_t int_dist, std::size_t i, std::vector<int> const& spin, Lattice::Lattice* lattice, double J1=0, double J2=0, double J3=0, double J4=0, double Jp=0){
...
int main(int argc, char **argv){
  ...
  double J4 = config->get_as<double>("J4").value_or(0.0);  
  ...
  bool measure_domain = config->get_as<bool>("measure_domain").value_or(false);
  std::vector<std::string> domain_types = split_string(*(config->get_as<std::string>("domain_types")), ' ');
  std::size_t neighborhood_range = config->get_as<int64_t>("neighborhood_range").value_or(2);  
  ...
  std::size_t interaction_distance = config->get_as<int64_t>("interaction_distance").value_or(4);
  int precision = 10;
  double beta = 1. / kT;
  ...
  std::vector<std::string> available_domain_types = {"2by2", "2by1", "4by4"};
  for(std::string type: domain_types){
    if ( !is_elem_in_vector(available_domain_types, type) ) {
      std::cerr << "Domain type " << type << " is not supported.\n";
    }
  }
  ...
  for(std::size_t task: tasks){
    ...
    /* For measurements */
    std::vector<cx_double> phase_pi_pi(N);
    std::vector<cx_double> phase_pi_0(N);
    std::vector<cx_double> phase_0_pi(N);
    std::vector<cx_double> phase_pi2_pi2(N);
    std::vector<cx_double> phase_pi2_mpi2(N);
    auto calc_phases = [&](){
      std::vector<double> q_pi_pi({M_PI, M_PI});      
	...
	phase_pi_pi[i] = lattice->phase(i, q_pi_pi);
	phase_pi_0[i] = lattice->phase(i, q_pi_0);
	phase_0_pi[i] = lattice->phase(i, q_0_pi);
	phase_pi2_pi2[i] = lattice->phase(i, q_pi2_pi2);
	phase_pi2_mpi2[i] = lattice->phase(i, q_pi2_mpi2);	
      }
    };
    calc_phases();
    ...
    ofstream op_2by2_out(obs_dir/"op_2by2.text");    
    ofstream op_2by1_out(obs_dir/"op_2by1.text");
    ofstream op_4by4_out(obs_dir/"op_4by4.text");
    ofstream largest_domain_2by2_out;
    ofstream second_largest_domain_2by2_out;
    ofstream largest_domain_2by1_out;
    ofstream second_largest_domain_2by1_out;
    ofstream largest_domain_4by4_out;
    ofstream second_largest_domain_4by4_out;
    
    /* Setting the precision */
    e_out << std::setprecision( precision );
    m_out << std::setprecision( precision );
    op_2by2_out << std::setprecision( precision );        
    op_2by1_out << std::setprecision( precision );
    op_4by4_out << std::setprecision( precision );        

    /* Result */
    std::vector<double> es(buffer_size);
    std::vector<double> ms(buffer_size);
    std::vector<double> op_2by2(buffer_size);        
    std::vector<double> op_2by1(buffer_size);
    std::vector<double> op_4by4(buffer_size);
    std::vector<std::size_t> largest_domain_2by2;
    std::vector<std::size_t> second_largest_domain_2by2;
    std::vector<std::size_t> largest_domain_2by1;
    std::vector<std::size_t> second_largest_domain_2by1;
    std::vector<std::size_t> largest_domain_4by4;
    std::vector<std::size_t> second_largest_domain_4by4;    
    
    if ( measure_domain ) {
      if ( is_elem_in_vector(domain_types, "2by2") ) {
	largest_domain_2by2_out.open(obs_dir/"largest_domain_2by2.text");
	second_largest_domain_2by2_out.open(obs_dir/"second_largest_domain_2by2.text");
	largest_domain_2by2.resize(buffer_size);
	second_largest_domain_2by2.resize(buffer_size);	
      }
      if ( is_elem_in_vector(domain_types, "2by1") ) {
	largest_domain_2by1_out.open(obs_dir/"largest_domain_2by1.text");
	second_largest_domain_2by1_out.open(obs_dir/"second_largest_domain_2by1.text");
	largest_domain_2by1.resize(buffer_size);
	second_largest_domain_2by1.resize(buffer_size);	
      }
      if ( is_elem_in_vector(domain_types, "4by4") ) {
	largest_domain_4by4_out.open(obs_dir/"largest_domain_4by4.text");
	second_largest_domain_4by4_out.open(obs_dir/"second_largest_domain_4by4.text");
	largest_domain_4by4.resize(buffer_size);
	second_largest_domain_4by4.resize(buffer_size);      	
      }
    }
    ...
    if ( dump_config_depend_on_order_param ) {
      if ( dump_controlling_parameter == "2by2" ) {
	dump_controller = &op_2by2;
      } else if ( dump_controlling_parameter == "2by1" ) {
	dump_controller = &op_2by1;
    ...
    auto do_measurements = [&](std::size_t index){
      ...
      if ( measure_energy ) {
	for(std::size_t i=0; i < N; i++){	    
	  double hi = calc_eff_local_field(fast_code, interaction_distance, i, spin, lattice.get(), 0.5*J1, 0.5*J2, 0.5*J3, 0.5*J4, 0.25*Jp);	  
	  double ei = - hi * spin[i];
	  e += ei;
	}
      }
      ...
      for(std::size_t i=0; i < N; i++){	
	m += (double)spin[i];
	m_pi_pi += (double)spin[i] * phase_pi_pi[i];
	m_pi_0 += (double)spin[i] * phase_pi_0[i];
	m_0_pi += (double)spin[i] * phase_0_pi[i];
	m_pi2_pi2 += (double)spin[i] * phase_pi2_pi2[i];
	m_pi2_mpi2 += (double)spin[i] * phase_pi2_mpi2[i];
      }
      ...
      m_pi2_pi2 /= (double)N;
      m_pi2_mpi2 /= (double)N;
      op_2by2[index] = sqrt(std::norm(m_pi_pi));      
      op_2by1[index] = sqrt(std::norm(m_0_pi) + std::norm(m_pi_0));
      op_4by4[index] = sqrt(2.*(std::norm(m_pi2_pi2) + std::norm(m_pi2_mpi2)));  // Multiplied by 2
    };

    /* Domain */    
    auto measure_domain_size = [&](std::size_t index){
      std::size_t rank = 2;  /* Finding the rank largest domains */      
      int up = 1;
      int down = -1;
      for(std::string domain_type: domain_types){
      	std::vector<std::size_t> size_max_vec;
      	if ( domain_type == "2by2" ) {
      	  std::vector<int> ref_state = Lattice::SquareLattice::make_2by2_state(spin.size(), up, down);
      	  size_max_vec = find_largest_domain_sizes(*lattice, spin, ref_state, neighborhood_range, rank);
	  largest_domain_2by2[index] = size_max_vec[0];
	  second_largest_domain_2by2[index] = size_max_vec[1];
      	} else if ( domain_type == "2by1" ) {
	  /* Finding the largest domains for each ordered configuration type. */
      	  std::size_t n_state_types = 2;
      	  std::vector<std::vector<std::size_t>> size_max_vecs;
      	  for(std::size_t type = 0; type < n_state_types; type++){
      	    std::vector<int> ref_state = Lattice::SquareLattice::make_2by1_state(spin.size(), up, down, type);
      	    std::vector<std::size_t> size_max_vec_type = find_largest_domain_sizes(*lattice, spin, ref_state, neighborhood_range, rank);
      	    size_max_vecs.push_back(size_max_vec_type);
      	  }

	  /* Picking out the largest ones. */
      	  size_max_vec = picking_out_largest(size_max_vecs, rank);
	  largest_domain_2by1[index] = size_max_vec[0];
	  second_largest_domain_2by1[index] = size_max_vec[1];	  
      	} else if ( domain_type == "4by4" ) {
	  /* Finding the largest domains for each ordered configuration type. */	  
      	  std::size_t n_state_types = 4;
      	  std::vector<std::vector<std::size_t>> size_max_vecs;
      	  for(std::size_t type = 0; type < n_state_types; type++){
      	    std::vector<int> ref_state = Lattice::SquareLattice::make_4by4_state(spin.size(), up, down, type);	    
      	    std::vector<std::size_t> size_max_vec_type = find_largest_domain_sizes(*lattice, spin, ref_state, neighborhood_range, rank);
      	    size_max_vecs.push_back(size_max_vec_type);
      	  }
	  ...
	  /* Picking out the largest ones. */	  
      	  size_max_vec = picking_out_largest(size_max_vecs, rank);
	  largest_domain_4by4[index] = size_max_vec[0];
	  second_largest_domain_4by4[index] = size_max_vec[1];	  
      	}	
      }
    };
	...
	op_2by2_out << tp << std::setw(precision + 10) << op_2by2[ti] << std::endl;	
	op_2by1_out << tp << std::setw(precision + 10) << op_2by1[ti] << std::endl;
	op_4by4_out << tp << std::setw(precision + 10) << op_4by4[ti] << std::endl;

	if ( measure_domain ) {
	  if ( is_elem_in_vector(domain_types, "2by2") ) {
	    largest_domain_2by2_out << tp << std::setw(precision + 10) << largest_domain_2by2[ti] << std::endl;
	    second_largest_domain_2by2_out << tp << std::setw(precision + 10) << second_largest_domain_2by2[ti] << std::endl;	    
	  }
	  if ( is_elem_in_vector(domain_types, "2by1") ) {
	    largest_domain_2by1_out << tp << std::setw(precision + 10) << largest_domain_2by1[ti] << std::endl;
	    second_largest_domain_2by1_out << tp << std::setw(precision + 10) << second_largest_domain_2by1[ti] << std::endl;	    
	  }
	  if ( is_elem_in_vector(domain_types, "4by4") ) {
	    largest_domain_4by4_out << tp << std::setw(precision + 10) << largest_domain_4by4[ti] << std::endl;
	    second_largest_domain_4by4_out << tp << std::setw(precision + 10) << second_largest_domain_4by4[ti] << std::endl;	    
	  }
	}
      }
    };
	...
	double hi = calc_eff_local_field(fast_code, interaction_distance, r, spin, lattice.get(), J1, J2, J3, J4, Jp);
    ...
    op_4by4_out.close();
    if ( measure_domain ) {
      largest_domain_2by2_out.close();
      second_largest_domain_2by2_out.close();
      largest_domain_2by1_out.close();
      second_largest_domain_2by1_out.close();
      largest_domain_4by4_out.close();
      second_largest_domain_4by4_out.close();
    }
  } /* end for task */
